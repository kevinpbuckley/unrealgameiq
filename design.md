...
- **Editor UI — Tools ▸ Game IQ:** a dockable Slate panel (`SGameIQPanel`) showing index stats (project, DB size, last-built time, entity counts by kind, edge count) with a **Rebuild Index** button that runs `GameIQBuild` in-process. Registered as a nomad tab + Tools-menu entry from the module.
