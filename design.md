...
- **Native toolset `UGameIQService : UToolsetDefinition`** (`plugin/GameIQ`, registered via `UToolsetRegistry::RegisterToolsetClass` on `PostEngineInit`) — zero-setup for anyone on the native UE MCP / VibeUE. Its `AICallable` functions (`Search`, `GetEntity`, …) are answered **in-process** by a C++ port of the query engine (`GameIQQuery`) reading the *same* `.gameiq/index.db` via UE's **SQLiteCore** (FTS5) — no Node spawn, no per-call editor-API round-trips, so it returns project data faster than probing the editor with many calls. Verified live on ThirdPerson58 via `describe_toolset` and `call_tool`.
- **DB coexistence:** the index uses a **rollback journal (`journal_mode=DELETE`), not WAL** — on Windows two different SQLite builds (Node's better-sqlite3 and UE's embedded SQLite) can't share a WAL's `-shm` memory (opens fail with "disk I/O error"). A rollback journal has no `-shm`, so both readers coexist. `FSQLiteDatabase` also asserts if destroyed while open, so the toolset closes its handle on every path (`ON_SCOPE_EXIT`).
- **Coverage & roadmap:** Phase 1 ships `Search` + `GetEntity`. Phase 2 ports the remaining actions (`references`/`impact`/`explain`/`project_stats`). Phase 3 (the freshness fork): today the save hook writes JSON deltas that only the Node side ingests into SQLite, so the in-editor toolset reflects live saves only after a Node ingest; porting `ingest`/`patchIngest` to C++ so the save hook writes SQLite directly would make the in-editor path fully self-contained (npm-free).
...
- **Incremental updates (§8):** in-editor save hook (`UGameIQSaveHookSubsystem`) writes a delta on any asset save (Blueprints and non-Blueprints via the shared recipes) and on delete/rename (Asset Registry delegates); MCP server drains pending deltas before each query (entity-scoped patch). Verified live: variable-value change, non-BP save, delete, and rename all reflect on the next query, no full rebuild.
