...
**Known gaps** — base-material texture refs only via Tier 0 `depends-on` (headless `GetUsedTextures` returns empty; MI overrides are typed); level per-actor entities capped at 500, `get_entity` children capped at 50 (use `search`/`references` for the full set; full class counts always reported, §12.7); Blueprint input bindings wired in a native C++ base class aren't in the graph (surfaced in the input-system logs — needs the C++ base's `SetupPlayerInputComponent`, a Tier-2/code-link improvement).
