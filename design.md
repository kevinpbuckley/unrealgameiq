...
**Decision: shipped as a *second front-end*, additive — the standalone stdio MCP server stays the independent product.** The two share one core (the SQLite index + schema); they differ only in who reads it:

- **Standalone stdio MCP (Node)** — headless, editor-optional, agent-agnostic, CI. This is what keeps Game IQ relevant even if VibeUE or Epic's toolset API changes; it rests on nothing Epic controls.
- **Native toolset `UGameIQService : UToolsetDefinition`** (`plugin/GameIQ`, registered via `UToolsetRegistry::RegisterToolsetClass` on `PostEngineInit`) — zero-setup for anyone on the native UE MCP / VibeUE. Its `AICallable` functions — the full query surface: `Search`, `GetEntity`, `References`, `Impact`, `Explain`, `ProjectStats` — are answered **in-process** by a C++ port of the query engine (`GameIQQuery`) reading the *same* `.gameiq/index.db` via UE's **SQLiteCore** (FTS5) — no Node spawn, no per-call editor-API round-trips, so it returns project data faster than probing the editor with many calls. Verified live on ThirdPerson58 via `describe_toolset` and `call_tool`.
- **The editor owns the whole pipeline (no Node required).** `FGameIQStore` is a C++ port of the store+ingest (schema in lockstep with `packages/core`, same rollback journal). The **save hook** patches the index directly on save/delete/rename (`FGameIQStore::Patch` — verified: a Blueprint variable value change reflects in the toolset immediately, no drain step). The **`GameIQIndex` commandlet** builds the full index from the extract JSONs (`FGameIQStore::IngestProducer`, producer-scoped merge — verified: 2219 entities from the three C++ producers, and it *preserves* other producers' rows). So building, live updates, and querying are all editor-native. A C++-written DB round-trips cleanly through Node's better-sqlite3 and vice-versa (same schema, `user_version=2`).
- **When is Node still needed?** Only two things remain TypeScript-only: (a) **editor-closed querying** — the standalone stdio server serves agents when no editor is running (CI, cold headless agents); (b) the **`gameiq-config`** producer (project-settings extraction, ~26 entities) has no C++ port yet, so a fully Node-free build omits it. Neither is needed for the in-editor product; both are the editor-optional independence hedge.
- **DB coexistence:** the index uses a **rollback journal (`journal_mode=DELETE`), not WAL** — on Windows two different SQLite builds (Node's better-sqlite3 and UE's embedded SQLite) can't share a WAL's `-shm` memory (opens fail with "disk I/O error"). A rollback journal has no `-shm`, so writers/readers across both libraries coexist (`busy_timeout` covers overlap). `FSQLiteDatabase` also asserts if destroyed while open, so every handle is closed on all paths (`ON_SCOPE_EXIT`).
- Read it competitively too: Epic's assistant + `AgentSkill` concept encroaches on all three Buckley Builds products at once (engine knowledge, project context, in-editor action) — a reason to *own* the cross-agent, editor-optional layer Epic won't build, while still meeting their users where they are.
...
- **MCP — two front-ends over one index (§6.5):** the standalone `game_iq` multi-action stdio tool (Node, headless/editor-closed; `claude mcp add`), **and** the native `UGameIQService` toolset on Epic's MCP endpoint (in-process C++ via SQLiteCore, zero-setup for UE-MCP/VibeUE agents). Full action parity: `search`, `get_entity`, `references`, `impact`, `explain`, `project_stats`.
- **Editor-native pipeline (§6.5):** `FGameIQStore` (C++ store+ingest) lets the editor own building *and* updating the index — no Node. The save hook patches SQLite directly on save/delete/rename; the `GameIQIndex` commandlet does the full build. Verified live: a Blueprint variable value change is visible in the toolset immediately; the C++ full build produces 2219 entities and its DB round-trips through Node's better-sqlite3.
